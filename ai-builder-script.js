...
// Timestamp Model Functions
function updateDateBasis() {
    const select = document.getElementById('dateBasisSelect');
    const selectedOption = select.options[select.selectedIndex];
    const value = selectedOption.value;
    const context = selectedOption.dataset.context;
    const accounting = selectedOption.dataset.accounting;
    
    // Update explanation text
    updateTimestampExplanation(value);
    
    // Update date range label
    updateDateRangeLabel(selectedOption.textContent);
    
    // Show/hide accounting notice
    updateAccountingNotice(accounting);
    
    // Apply context-aware filtering
    applyContextAwareFiltering(context);
    
    // Update report data
    reportData.dateBasis = value;
    
    // Trigger report refresh if components exist
    refreshReportComponents();
    
    showNotification(`Date basis updated to: ${selectedOption.textContent}`);
}

function updateTimestampExplanation(value) {
    const explanationText = document.getElementById('explanationText');
    
    const explanations = {
        'order_created': 'When the order was initiated. Represents demand creation and the start of the sale lifecycle.',
        'order_fulfilled': 'When goods or services were delivered. Critical for fulfillment-based and future accrual-aligned reporting.',
        'payment_authorized': 'When the customer presented payment and authorization was obtained. Funds are not yet guaranteed.',
        'payment_completed': 'When funds are finalized. Best aligns with cash movement and transfers.',
        'campaign_attribution': 'When the order was attributed to the campaign. Used for marketing performance analysis.',
        'inventory_event': 'When inventory changes occurred. Used for stock movement and availability analysis.',
        'visit_time': 'When the customer visit or session occurred. Used for traffic and engagement analysis.'
    };
    
    explanationText.textContent = explanations[value] || 'Select a timestamp to see explanation.';
}

function updateDateRangeLabel(timestampText) {
    const label = document.getElementById('dateRangeLabel');
    label.textContent = `Date range (based on ${timestampText})`;
}

function updateAccountingNotice(accounting) {
...
function applyContextAwareFiltering(context) {
    // This would filter available metrics and widgets based on context
    // For now, we'll just log the context change
    console.log('Context changed to:', context);
    
    // In a real implementation, this would:
    // 1. Filter available data sources in the Data tab
    // 2. Show/hide relevant widgets in the Widgets tab  
    // 3. Update available metrics based on selected timestamp context
    
    // Update AI suggestions based on context
    updateAISuggestionsForContext(context);
}

function updateAISuggestionsForContext(context) {
    const suggestions = document.querySelector('.ai-suggestions');
    
    // Clear existing suggestions
    suggestions.innerHTML = '';
    
    if (context && context.includes('marketing')) {
        // Marketing-focused suggestions
        suggestions.innerHTML = `
            <div class="suggestion-card" onclick="applySuggestion('campaign-attribution-analysis')">
                <i class="fas fa-bullhorn"></i>
                <div class="suggestion-content">
                    <h4>Campaign Attribution Analysis</h4>
                    <p>Analyze which campaigns are driving the most attributed sales</p>
                </div>
            </div>
            <div class="suggestion-card" onclick="applySuggestion('marketing-funnel-analysis')">
                <i class="fas fa-funnel-dollar"></i>
                <div class="suggestion-content">
                    <h4>Marketing Funnel Analysis</h4>
                    <p>Track customer journey from campaign interaction to purchase</p>
                </div>
            </div>
            <div class="suggestion-card" onclick="applySuggestion('roi-by-channel')">
                <i class="fas fa-chart-line"></i>
                <div class="suggestion-content">
                    <h4>ROI by Marketing Channel</h4>
                    <p>Compare return on investment across different marketing channels</p>
                </div>
            </div>
        `;
    } else if (context && context.includes('payment')) {
        // Payment-focused suggestions
        suggestions.innerHTML = `
            <div class="suggestion-card" onclick="applySuggestion('payment-reconciliation')">
                <i class="fas fa-balance-scale"></i>
                <div class="suggestion-content">
                    <h4>Payment Reconciliation Report</h4>
                    <p>Match payments with orders for accurate financial reporting</p>
                </div>
            </div>
            <div class="suggestion-card" onclick="applySuggestion('cash-flow-analysis')">
                <i class="fas fa-coins"></i>
                <div class="suggestion-content">
                    <h4>Cash Flow Analysis</h4>
                    <p>Track actual cash movement and payment timing</p>
                </div>
            </div>
            <div class="suggestion-card" onclick="applySuggestion('payment-method-performance')">
                <i class="fas fa-credit-card"></i>
                <div class="suggestion-content">
                    <h4>Payment Method Performance</h4>
                    <p>Analyze success rates and timing by payment method</p>
                </div>
            </div>
        `;
    } else if (context && context.includes('fulfillment')) {
        // Fulfillment-focused suggestions
        suggestions.innerHTML = `
            <div class="suggestion-card" onclick="applySuggestion('fulfillment-performance')">
                <i class="fas fa-shipping-fast"></i>
                <div class="suggestion-content">
                    <h4>Fulfillment Performance</h4>
                    <p>Track delivery times and fulfillment efficiency</p>
                </div>
            </div>
            <div class="suggestion-card" onclick="applySuggestion('delivery-analytics')">
                <i class="fas fa-truck"></i>
                <div class="suggestion-content">
                    <h4>Delivery Analytics</h4>
                    <p>Analyze delivery patterns and customer satisfaction</p>
                </div>
            </div>
            <div class="suggestion-card" onclick="applySuggestion('inventory-turnover')">
                <i class="fas fa-boxes"></i>
                <div class="suggestion-content">
                    <h4>Inventory Turnover Analysis</h4>
                    <p>Match fulfillment data with inventory levels</p>
                </div>
            </div>
        `;
    } else {
        // Default suggestions for order/sales context
        suggestions.innerHTML = `
...
function refreshReportComponents() {
    // This would refresh all report components to use the new timestamp basis
    // For now, we'll just update any existing widgets
    const widgets = document.querySelectorAll('.canvas-component.widget-component');
    
    widgets.forEach(widget => {
        // Add a visual indicator that the widget has been updated
...
function setDateRange(preset) {
    ...
    // Hide presets
    document.getElementById('datePresets').style.display = 'none';
    ...
    showNotification(`Date range set to: ${preset.replace(/([A-Z])/g, ' $1').toLowerCase()}`);
}
...
// Report Type Detection and Auto-Configuration
function detectReportTypeAndSetDefaults() {
    const title = document.querySelector('.title-input').value.toLowerCase();
    const dataSources = reportData.dataSources;
    
    let suggestedTimestamp = 'order_created'; // default
    
    // Detect based on title
    if (title.includes('payment') || title.includes('cash') || title.includes('payout')) {
        suggestedTimestamp = 'payment_completed';
    } else if (title.includes('marketing') || title.includes('campaign') || title.includes('attribution')) {
        suggestedTimestamp = 'campaign_attribution';
    } else if (title.includes('fulfillment') || title.includes('delivery') || title.includes('shipping')) {
        suggestedTimestamp = 'order_fulfilled';
    } else if (title.includes('inventory') || title.includes('stock')) {
        suggestedTimestamp = 'inventory_event';
    } else if (title.includes('traffic') || title.includes('visit') || title.includes('engagement')) {
        suggestedTimestamp = 'visit_time';
    }
    
    // Detect based on data sources
    if (dataSources.includes('payments') && !dataSources.includes('orders')) {
        suggestedTimestamp = 'payment_completed';
    } else if (dataSources.length === 1 && dataSources[0] === 'customers') {
        suggestedTimestamp = 'order_created'; // customer registration or first order
    }
    
    // Apply the suggested timestamp
    const select = document.getElementById('dateBasisSelect');
    if (select && select.value !== suggestedTimestamp) {
        select.value = suggestedTimestamp;
        updateDateBasis();
    }
}

// Initialize timestamp model when builder loads
function initializeTimestampModel() {
    // Set default date range to last 30 days
    setDateRange('last30days');
    
    // Set default timestamp based on template if available
    if (selectedTemplate) {
        let defaultTimestamp = 'order_created';
        
        switch (selectedTemplate) {
            case 'sales-performance':
            case 'sales-forecast':
                defaultTimestamp = 'order_created';
                break;
            case 'financial-overview':
                defaultTimestamp = 'payment_completed';
                break;
            case 'customer-insights':
                defaultTimestamp = 'order_created';
                break;
            case 'inventory-optimization':
                defaultTimestamp = 'inventory_event';
                break;
            case 'labor-analytics':
                defaultTimestamp = 'order_created'; // or could be shift_time in a real system
                break;
        }
        
        const select = document.getElementById('dateBasisSelect');
        if (select) {
            select.value = defaultTimestamp;
            updateDateBasis();
        }
    }
}

// Enhanced template selection to include timestamp defaults
const originalSelectTemplate = selectTemplate;
selectTemplate = function(templateId) {
    originalSelectTemplate(templateId);
    
    // Initialize timestamp model after template is loaded
    setTimeout(() => {
        initializeTimestampModel();
    }, 100);
};

// Add event listeners for timestamp model
document.addEventListener('DOMContentLoaded', function() {
    // Listen for title changes to auto-detect report type
    const titleInput = document.querySelector('.title-input');
    if (titleInput) {
        titleInput.addEventListener('blur', detectReportTypeAndSetDefaults);
    }
    ...
    if (startDate) {
        startDate.addEventListener('change', function() {
            reportData.dateRange = {
                ...
                end: reportData.dateRange ? reportData.dateRange.end : new Date(),
                preset: 'custom'
            };
        });
    }
    ...
    if (endDate) {
        endDate.addEventListener('change', function() {
            reportData.dateRange = {
                start: reportData.dateRange ? reportData.dateRange.start : new Date(),
